 ...
 * @author bruno
 */
public class ProcessingReducer extends Reducer<IntWritable, DriveLog, NullWritable, Text> {

    @Override
    protected void reduce(IntWritable key, Iterable<DriveLog> values, Context context) throws IOException, InterruptedException {
        double[][] amounts = new double[152][152];
        int[][] count = new int[152][152];

        for (DriveLog value : values) {
